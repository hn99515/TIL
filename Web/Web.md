...
> **HTML5에서 의미론적 요소를 담은 태그의 등장 (div 태그를 대체하여 사용)**
...
* **자손 결합자 (`공백`) - 선택자A 하위의 모든 선택자B 요소**

* **자식 결합자 (`>`) - 선택자A 바로 아래의 선택자B 요소**
...
> 모든 요소는 네모(박스 모델)이고,
> 
> **위에서부터 아래로, 왼쪽(좌측 상단)에서 오른쪽으로 쌓인다.**
...
* **box sizing**
  ...
  * 일반적으로 영역을 볼 때는 border까지의 너비를 표현
  
  * **box-sizing의 기준을 border-box로 설정! - content 크기로 width 설정 가능!**
...
* **display: inline**
  ...
  * width, height, margin-top, margin-bottom 지정 불가 = 상하여백은 line-height로 지정
...
* 좌표 프로퍼티(top, bottom, left, right)를 사용하여 이동
  ...
  * `absolute` - 절대 위치
    ...
    * static이 아닌 가장 가까이 있는 부모/조상 요소를 기준으로 이동 (업으면 브라우저 화면 기준으로 이동)
  ...
  * `sticky` - 스크롤 이동에 따라 static -> fixed로 변경
    ...
    * 속성을 적용한 박스는 평소에 문서 안에서 position: static 상태와 같이 일반적인 흐름이지만 스크롤 위치가 임계점에 이르면 position: fixed와 같이 박스를 화면에 고정할 수 있음
