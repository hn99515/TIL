...
* 각각의 객체는 메시지를 주고 받을 수 있으며, 데이터를 처리할 수 있다. = 상호작용!
...
* **객체와 인스턴스**
  ...
  * 예시
    ...
    * 가수 '000' 의 인스턴스다 (O) = 특정 클래스의 인스턴스다.

* **객체(object)는 특정 타입(class)의 인스턴스다.**
...
* 기본 문법 = 클래스 정의 + 인스턴스 생성 + 메서드 호출 + 속성
  ...
  ❗ 클래스 네이밍 규칙 - pascal / 함수 네이밍 규칙 = snake ❗

* **파이썬은 모든 것이 객체이며, 모든 객체는 특정 타입의 인스턴스다**‼
...
* 객체 비교하기
  ...
  * **`==`**
    ...
    * 두 객체가 같아 보이지만 <u>*실제로 동일한 대상을 가리키고 있다고 확인한 것은 아님*</u>
  
  * **`is`**
    
    * 동일함(identical)
    
    * 두 변수가 동일한 객체(id값 같음)를 가리키는 경우 - True
...
print(c1.pi) # 3.14 # 이름 찾기 규칙에 따라 클래스 변수가 나온 
print(c2.pi) # 3.14
...
* **클래스 변수 변경법 = 항상 `클래스.클래스변수` 형식으로 변경!**
